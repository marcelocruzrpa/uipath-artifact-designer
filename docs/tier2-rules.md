...
| ---: | --- | --- | --- | --- | --- | --- |
| 1 | `assign-from-call` *(proposed)* | assign | floor (top bucket #8) | 51 / 16 | Spec floor. `x = Foo(...)` / `var x = Obj.Foo(...)` is the single most common honest pseudo-step ("Assign x from call"); generically absorbs project-local factories (TestHelpers.\*, 27) without naming them. | Receiver chain must root at an identifier or static type name; calls chained on expressions (`new Random().Next(...)`) and `return Foo(...)` stay tier-3. |
| 2 | `string-op` *(proposed)* | string | floor (top bucket #19) | 20 / 13 | Spec floor. Single string operation assigns/appends: `Trim`/`IndexOf`/`ToString`, `+`/`+=` concatenation, `$"..."` interpolation initializers, StringBuilder `Append`/`AppendLine`. | Fluent chains of ≥2 ops beyond a trailing case fold, and format expressions nested inside other calls. |
