 ...
 *
 * Editing targets the RAW SOURCE TOKEN (`arg.valueRaw`), never the clipped /
 * unquoted display value (`arg.value`): the host resolves the edit against the
 * arg's `valueSpan`, so the text round-tripped through the input must be the
 * exact source slice. An arg is editable only when ALL hold:
...
export function renderPropertiesPanel(
  ...
  card.args.forEach((arg, argIndex) => {
    ...
    input.type = 'text';
    input.value = arg.valueRaw ?? arg.value;
